...
class MyLogger():
  ...
  def progress(self, data, every=0, finalItem=False, newLine=True, noBlank=False):
    with lock:
  ...
  def out(self, data, newLine=True, log=False, padspaces=True):
    with lock:
      udata = MyUtility.toUnicode(data)
      ulen = len(data)
      spaces = self.lastlen - ulen
  ...
  def err(self, data, newLine=True, log=False):
    with lock:
      self.progress("")
      udata = MyUtility.toUnicode(data)
      if newLine:
